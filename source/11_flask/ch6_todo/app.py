...
from flask import redirect, url_for, abort # 강제 예외처리
from flask import session # 로그인/로그아웃
from models import Todo

app = Flask(__name__)
# app.sectret_key = 'abc123!' # 세션을 사용할 경우 필수. 아무거나 써도됨
...
@app.route('/logout')
def logout():
    "로그아웃 로직(세션에 정보 제거)후 /todos로 이동"
    session.pop('user_id', None) # 딕셔너리라 날릴떄는 pop으로 날림
