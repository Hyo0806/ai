...
# 1. form없이 그냥 사용
# 2. form 객체 생성후(6장)
# 3. DjangoGenericView 이용(7장) 
# 4. GenericView 상속(7장)

# def book_list(request):
#     return render(request, "book/book_list.html", {"book_list":Book.objects.all(), 'object_list':Book.objects.all()})
...
book_new1 = CreateView.as_view(model=Book, fields = ['title', 'author', 'publisher', 'sales'])
...
def book_edit1(request, pk):
    # GET  : 수정 template페이지만(book) 응답
    # POST : 파라미터 받아 유효성 체크
        # 1) success : db에 update → book:list
        # 2) fail : 요류메세지가 담긴 form객체와 함께 수정 template페이지로 이동
...
book_delete = DeleteView.as_view(model=Book, success_url = reverse_lazy('book:list')) # delete후에 변환하여 실행