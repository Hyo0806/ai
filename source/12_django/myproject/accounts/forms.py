...
class SignupForm(UserCreationForm):
    phone_number = forms.CharField(label='전화', max_length=20,
                                help_text='전화번호는 필수 입력사항이 아닙니다',
                                required=False)
    address      = forms.CharField(label='주소', max_length=100,
                                help_text='주소는 필수 입력사항이 아닙니다',
                                required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['password1'].help_text = '2글자 이상의 안전한 비밀번호를 입력하세요'
        self.fields['password2'].help_text = '동일한 비밀번호를 입력하세요'
        self.fields['email'].help_text = '이메일은 필수 입력이 아닙니다'

    def save(self, commit=True):
        user = super().save() # auth_user 테이블에 저장(username, password)
